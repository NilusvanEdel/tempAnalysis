...
child_raw.plot <- ggplot(child.cont, aes(x = perspective,
    ...
    ylab("Confidence") +
     geom_errorbar(aes(ymin = confidence_signed - confidence_sd,
                       ymax = confidence_signed + confidence_sd, width = 0.1),
               data = child.cont) +
    scale_x_discrete(name = "Perspective",
                     limits = levels(child.cont$perspective),
    ...
    scale_shape_manual(values = c(22, 15), name = "Mean (\u00B1SD) confidence in decision",
                       labels = c("Endanger child pedestrians (fewer lives)",
                                  "Endanger adult pedestrians (more lives)"),
                       limits = rev(levels(child.cont$decision))) +
    scale_fill_manual(values = c("#67a9cf", "#ef8a62", "black", "white"),
                      name = "Relative frequency of decision",
                      labels = c("Endanger child pedestrians (fewer lives)",
                                 "Endanger adult pedestrians (adult lives)")) +
            geom_point(stat = "identity", aes(y = confidence_signed,
                                      group = decision:perspective,
