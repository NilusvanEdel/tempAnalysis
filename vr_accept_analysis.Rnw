...
child.sub$confidence_signed <- ifelse(child.sub$decision == "hitChildren", -1*child.sub$confidence,child.sub$confidence)
child_raw.plot <- ggplot(child.sub, aes(perspective)) + geom_bar(aes(y = ..prop.., group = decision:motorist, fill = decision)) + scale_y_continuous(sec.axis = sec_axis(~.*200-100, name = "confidence")) + stat_summary(aes(y=confidence_signed/200+0.5, shape = decision), data = child.sub) + geom_hline(yintercept = 0.5, linetype = "dashed") + scale_shape_manual(values = c(60,62)) + coord_flip()
...
<<carsac-mosaic>>=
carsac.xtabs <- xtabs(~ motorist + perspective + decision, data = carsac.sub)
