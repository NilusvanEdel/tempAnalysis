...
child_raw.plot <- ggplot(child.cont, aes(x = perspective,
    ...
    geom_bar(stat = "identity", aes(y = Freq, fill = decision), colour = "black", show.legend = FALSE) +
    ylab("Confidence") +
...
#    geom_hline(yintercept = 0, linetype = "dashed") +
    scale_shape_manual(
        ...
        labels = c("Endanger fewer lives",
                   "Endanger more lives"),
        limits = rev(levels(child.cont$decision))) +
    scale_fill_manual(
#        values = c("#969696", "#525252","white", "black"),  # black and white
        values = c("#FB6A4A", "#A50F15",  NA, "white"),
        name = "Relative frequency of decision",
        labels = c("Endanger fewer pedestrains",
...
#    geom_hline(yintercept = 0, linetype = "dashed") +
    ...
    scale_fill_manual(
        values = c( "#fdb863", "#b2abd2", NA, "white"),
        name = "Relative frequency of decision",
        labels = c("Endanger pedestrians",
                   "Endanger car occupants", "", "")) +
...
# joint plot
carsac_joint.plot <- ggarrange(carsac_city_raw.plot,
...
#    geom_hline(yintercept = 0, linetype = "dashed") +
    scale_shape_manual(
        ...
        labels = c("Endanger fewer pedestrians",
                   "Endanger more pedestrians")) +
    scale_fill_manual(
#        values = c( "#fdb863", "#b2abd2", "white", NA),
        values = c("#FB6A4A", "#A50F15", NA, "white"),
        name = "Relative frequency of decision",
        labels = c("Endanger fewer pedestrians",
                   "Endanger more pedestrians", "", "")) +
    ...
    guides(fill = guide_legend(order = 1),
           shape = guide_legend(order = 2)) +
        coord_flip() + theme_cowplot(font_size = 9) + theme(legend.position = "bottom",
                         legend.direction = "vertical")


# combine child and sidewalk plots

child_sidewalk.plot <- ggarrange(child_raw.plot, sidewalk_raw.plot, common.legend = TRUE,
                                 nrow = 2,
                                 legend = "bottom", labels = c("A", "B"))


# combine all figures for VR

vr_combined.plot <- ggarrange(child_sidewalk.plot, carsac_joint.plot)
