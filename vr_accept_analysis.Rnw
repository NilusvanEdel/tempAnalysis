...
child.sub$perspective <- factor(child.sub$perspective, levels = c("Observer", "Passenger", "PedSmall", "PedLarge"))
...
child_raw.plot <- ggplot(child.cont, aes(x = perspective,
    ...
    scale_x_discrete(name = "Perspective",
        ...
        geom_point(stat = "identity", aes(y = confidence_signed,
                                      group = decision:perspective,
                                          shape = decision), size = 2.5) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    scale_shape_manual(values = c(15, 22), name = "Mean (\u00B1SD) confidence in decision",
                       labels = c("Endanger adults (more lives)",
                                  "Endanger children (fewer lives)")) +
    scale_fill_manual(values = c("#ef8a62","#67a9cf", "black", "white"),
                      name = "Relative frequency of decision",
                      labels = c("Endanger adults (more lives)",
                                 "Endanger children (fewer lives)")) +
            geom_point(stat = "identity", aes(y = confidence_signed,
                                      group = decision:perspective,
                                          fill = decision2, shape = decision), size = 3, show.legend = FALSE) +
...
child_raw.plot <- ggplot(child.cont, aes(x = perspective,
    ...
    stat_summary(fun.y = mean, geom = "point",
                 aes(y = confidence_signed, shape = decision),
                 data = child.sub, size = 3) +
    ...
    scale_shape_manual(values = c(0, 15), name = "Mean (\u00B1SD) confidence in decision",
                       ...
                       labels = c("Endanger children (fewer lives)", "Endanger adults (more lives)")) +
   scale_fill_manual(name = "Relative frequency of decision",
                     labels = c("Endanger children (fewer lives)", "Endanger adults (more lives)"),
                     limits = rev(levels(child.cont$decision)),
...
carsac.cont$Freq <- ifelse(carsac.cont$decision == "hitPedestrians",
                             -carsac.cont$Freq, carsac.cont$Freq)

carsac.sub$confidence_signed <- ifelse(carsac.sub$decision == "hitPedestrians",
...
carsac_raw.plot <- ggplot(carsac.cont, aes(x = perspective,
    ...
    scale_x_discrete(name = "Perspective",
                     ...
                     labels = c("Pedestrian", "Passenger", "Bystander")) +
        scale_y_continuous(limits = c(-110,110),
                       sec.axis = sec_axis(~., name = "Relative frequency",
                                           labels = c("100%", "50%", "0%", "50%", "100%")),
                       labels = c(100, 50, 0, 50, 100)) +
    ...
    geom_hline(yintercept = 0, linetype = "dashed") +
    scale_shape_manual(values = c(22, 15), name = "Mean (\u00B1SD) confidence in decision",
                       labels = c("Endanger pedestrians",
                                  "Endanger passengers")) +
    scale_fill_manual(values = c("#67a9cf", "#ef8a62", "white", "black"),
                      name = "Relative frequency of decision",
                       labels = c("Endanger pedestrians",
                                  "Endanger passengers")) +
            geom_point(stat = "identity", aes(y = confidence_signed,
                                      group = decision:perspective,
                                          fill = decision2, shape = decision2), size = 3, show.legend = FALSE) +
    facet_grid(motorist ~ ., labeller = as_labeller(motorist_names)) +
        guides(fill = guide_legend(order = 1),
           shape = guide_legend(order = 2)) +
    coord_flip() + theme(legend.position = "bottom", legend.direction = "vertical")
...
carsac_raw.plot <- ggplot(carsac.cont, aes(x = perspective,
    ...
    stat_summary(fun.ymin = function(x) mean(x) - sd(x),
               ...
               geom = "linerange",
               aes(y = confidence_signed, group = decision:perspective),
               data = carsac.sub) +
    stat_summary(fun.y = mean, geom = "point",
                 aes(y = confidence_signed, color = decision),
                 data = carsac.sub, size = 3) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    scale_color_manual(values = c("black", "grey")) +
    scale_shape_manual(values = c("c", "c"),
                       name = "Mean (\u00B1SD) confidence in decision",
                       limits = rev(levels(carsac.cont$decision)),
...
@
TODO: Add follow analyses
<<carsac-conf-followup>>=
@
...
sidewalk.sub$confidence_signed <- ifelse(sidewalk.sub$decision == "hitSidewalk",
...
sidewalk.ctab <- ctab(sidewalk.sub$motorist, sidewalk.sub$perspective,
...
sidewalk_raw.plot <- ggplot(sidewalk.cont, aes(x = perspective,
    ...
    geom_hline(yintercept = 0, linetype = "dashed") +
    scale_shape_manual(values = c(22, 15), name = "Mean (\u00B1SD) confidence in decision",
                       labels = c("Endanger pedestrians on sidewalk (fewer lives)",
                                  "Endanger pedestrians on road (more lives)")) +
    scale_fill_manual(values = c("#67a9cf", "#ef8a62", "white", "black"),
                      name = "Relative frequency of decision",
                      labels = c("Endanger pedestrians on sidewalk (fewer lives)",
                                 "Endanger pedestrians on road (more lives)")) +
