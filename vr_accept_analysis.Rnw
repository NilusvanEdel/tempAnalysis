...
child.sub <- subset(child.data, child.data$passedSanCheck == "True" & child.data$SecondSanCheck == "True")
...
carsac.sub <- subset(carsac.data, carsac.data$passedSanCheck == "True" & carsac.data$SecondSanCheck == "True")
...
sidewalk.data$confidence <- as.numeric(sidewalk.data$confidence)
                                        ...
                                        # sanity check
sidewalk.sub <- subset(sidewalk.data, sidewalk.data$passedSanCheck == "True" & SecondSanCheck == "True")
...
<<child-mosaic>>=
child.xtabs <- xtabs(~ motorist + perspective + decision, data = child.sub)

child.cont <- data.frame(child.xtabs)

child.cont$Freq <- ifelse(child.cont$decision == "hitAdults", -child.cont$Freq, child.cont$Freq)

child.cont$Freq <- child.cont$Freq/46*100


child.sub$confidence_signed <- ifelse(child.sub$decision == "hitAdults", -child.sub$confidence, child.sub$confidence)


child_raw.plot <- ggplot(child.cont, aes(perspective)) +
    geom_bar(stat = "identity", aes(y = Freq, group = decision:motorist, fill = decision)) + ylab("Proportion of decisions") +
    scale_y_continuous(limits = c(-100,100), sec.axis = sec_axis(~., name = "Confidence in decision")) +
    stat_summary(aes(y = confidence_signed, shape = decision), data = child.sub) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    scale_shape_manual(values = c(15,15)) + coord_flip() + facet_grid(motorist ~ .)
...
carsac.xtab <- xtabs(~ motorist + perspective + decision, data = carsac.sub)

carsac.cont <- data.frame(carsac.xtab)

carsac.cont$Freq <- ifelse(carsac.cont$decision == "hitPedestrians", -carsac.cont$Freq, carsac.cont$Freq)

carsac.cont$Freq <- carsac.cont$Freq/46*100

carsac.sub$confidence_signed <- ifelse(carsac.sub$decision == "hitPedestrians", 1*carsac.sub$confidence,-1*carsac.sub$confidence)

carsac_raw.plot <- ggplot(carsac.cont, aes(perspective)) +
    geom_bar(stat = "identity", aes(y = Freq, group = decision:motorist, fill = decision)) + ylab("Proportion of decisions") +
    scale_y_continuous(limits = (c(-100, 100)), sec.axis = sec_axis(~., name = "Confidence in decision")) +
    stat_summary(aes(y = confidence_signed, shape = decision), data = carsac.sub) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    scale_shape_manual(values = c(15,15,15,15)) + coord_flip() + facet_grid(motorist ~ .)
...
sidewalk.xtabs <- xtabs(~ motorist + perspective + decision, data = sidewalk.sub)

sidewalk.cont <- data.frame(sidewalk.xtabs)

sidewalk.cont$Freq <- ifelse(sidewalk.cont$decision == "hitAdults", -sidewalk.cont$Freq, sidewalk.cont$Freq)

sidewalk.cont$Freq <- sidewalk.cont$Freq/46*100


sidewalk.sub$confidence_signed <- ifelse(sidewalk.sub$decision == "hitAdults", -sidewalk.sub$confidence, sidewalk.sub$confidence)


sidewalk_raw.plot <- ggplot(sidewalk.cont, aes(perspective)) +
    geom_bar(stat = "identity", aes(y = Freq, group = decision:motorist, fill = decision)) + ylab("Proportion of decisions") +
    scale_y_continuous(limits = c(-100,100), sec.axis = sec_axis(~., name = "Confidence in decision")) +
    stat_summary(aes(y = confidence_signed, shape = decision), data = sidewalk.sub) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    scale_shape_manual(values = c(15,15)) + coord_flip() + facet_grid(motorist ~ .)
